...
/**
 * Created by jetcai1900 on 1/4/17.
 */
public class CorrDCMLDA_Multi_EM extends CorrDCMLDA {
    public enum RunType {
        RT_inference, RT_E, RT_M, RT_EM
    }

    CorrDCMLDA_Multi_EM.RunType m_type = CorrDCMLDA_Multi_EM.RunType.RT_EM;// EM is the default type

    protected CorrDCMLDA_Multi_EM_worker[] m_multiEM_worker;

    public class CorrDCMLDA_Multi_EM_worker implements multiEM_worker {
        ...
        public CorrDCMLDA_Multi_EM_worker(int number_of_topics,
                                         int vocabulary_size) {
        ...
        public void run() {
            ...
            if(m_type == CorrDCMLDA_Multi_EM.RunType.RT_EM){
                System.out.println("EM error mode");
            }

            if(m_type == CorrDCMLDA_Multi_EM.RunType.RT_E){
//				System.out.println("E step");
                for(_Doc d:m_corpus){
                    calculate_E_step(d);
                }
            }else if(m_type == CorrDCMLDA_Multi_EM.RunType.RT_M){
...
//			System.out.println("iteration\t"+iteration);
        ...
        public void setType(CorrDCMLDA_Multi_EM.RunType type) {
            // TODO Auto-generated method stub
            m_type = type;
        }
    ...
    public CorrDCMLDA_Multi_EM(int number_of_iteration, double converge,
...
//		 m_multithread = true;
    ...
    public String toString() {
        return String
                .format("CorrDCMLDA_Multi_EM[k:%d, alphaA:%.2f, beta:%.2f, Gibbs Sampling]",
    ...
    protected void initialize_probability(Collection<_Doc> collection) {
        ...
        m_multiEM_worker = new CorrDCMLDA_Multi_EM_worker[cores];

        for (int i = 0; i < cores; i++)
            m_multiEM_worker[i] = new CorrDCMLDA_Multi_EM_worker(number_of_topics, vocabulary_size);
    ...
    protected void init() {
        // super.init();
        for (CorrDCMLDA_Multi_EM_worker worker : m_multiEM_worker) {
            worker.resetStats();
        }
    }
    ...
    protected void updateBeta() {
        for (int i = 0; i < m_multiEM_worker.length; i++) {
            m_multiEM_worker[i].setType(CorrDCMLDA_Multi_EM.RunType.RT_M);
...
//		saveParameter2File(weightIterFolder, fileName);
    ...
    protected void multithread_E_step() {
        for (int i = 0; i < m_multiEM_worker.length; i++) {
            m_multiEM_worker[i].setType(CorrDCMLDA_Multi_EM.RunType.RT_E);
            m_threadpool[i] = new Thread(m_multiEM_worker[i]);
            m_threadpool[i].start();
        }
