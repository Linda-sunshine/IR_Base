...
public class DCMLDA_multi_M extends DCMLDA{
	...
	public class DCMLDA_multi_mWorker extends updateParam_worker{
		...
		public void returnParameter(double[] a_param, int a_index){
			if(a_param.length==m_param.get(a_index).length){
				System.arraycopy(a_param, 0, m_param.get(a_index), 0, a_param.length);
			}
		}
	...
	public DCMLDA_multi_M(int number_of_iteration, double converge, double beta, 
			_Corpus c, double lambda, int number_of_topics, double alpha, double burnIn, 
 int lag, int newtonIter, double newtonConverge) {
		super( number_of_iteration,  converge,  beta,
				 c,  lambda,  number_of_topics,  alpha,
				 burnIn,  lag,  newtonIter,  newtonConverge);
	...
	protected void initialize_probability(Collection<_Doc> collection){
		...
		m_updateParam_workers = new DCMLDA_multi_mWorker[cores];
		
		for(int i=0; i<cores; i++)
			m_updateParam_workers[i] = new DCMLDA_multi_mWorker();
	...
	protected void updateBeta(){
		for(int i=0; i<m_updateParam_workers.length; i++){
			m_updateParam_workers[i].setType(RunType.RT_EM);
