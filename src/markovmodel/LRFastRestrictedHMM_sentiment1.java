...
public class LRFastRestrictedHMM_sentiment1 extends FastRestrictedHMM_sentiment1{
	...
	public LRFastRestrictedHMM_sentiment1 (double[] omega, double[] delta, int maxSeqSize, int topicSize) {
	...
	void initSigmas(_Doc d) {
		for(int t=1; t<d.getSenetenceSize(); t++)
			m_sigmas[t] = Utils.logistic(d.getSentence(t-1).getSentiTransitFvs(), m_delta);//first sentence does not have features
	}
