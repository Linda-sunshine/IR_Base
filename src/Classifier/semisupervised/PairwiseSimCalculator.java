...
public class PairwiseSimCalculator implements Runnable {
	...
	public void run() {
		_Doc di, dj;
		double similarity, topicSimilarity, discount = 0;
		for (int i = m_start; i < m_end; i++) {
			...
			for (int j = i + 1; j < m_GFObj.m_U; j++) {// to save computation since our similarity metric is symmetric
				...
				if(m_topicFlag && di.sameProduct(dj)){//If it is topic based similarity and they are for the same product
					similarity += 2 * topicSimilarity;	
				} else//if they are from different products, their topicSimilarity = 0.
					similarity += discount * topicSimilarity;
