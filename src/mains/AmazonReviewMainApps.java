...
public class AmazonReviewMainApps {
...
//		System.out.println("Feature Selection: " + featureSelection + "\tStarting probability: " + startProb + "\tEnding probability:" + endProb);
		...
		String resultPath = "./model/"+classifier+"/result/app/"+category+"_information.txt";
		...
		String featureDirectory = "./data/app/newdata/annotated/";
		...
		analyzer.LoadDirectory(folder, suffix, true); //Load all the documents as the data set.
		if(classifier.equalsIgnoreCase("EM-NB"))
		...
		analyzer = new appReviewAnalyzer(tokenModel, classNumber, fvFile, Ngram, lengthThreshold);
		analyzer.setCategory(category);
		analyzer.setClassifierOrTopicModel(true,stnModel);
